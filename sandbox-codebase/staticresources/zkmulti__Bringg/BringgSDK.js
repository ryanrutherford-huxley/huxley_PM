...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.BringgSDK = factory());
  ...
  var REAL_TIME_PRODUCTION = REGIONS["ue4g"];
  ...
  module.submitRating = function (rating) {
    if (configuration) {
      if (configuration.rating_url && configuration.rating_token) {
        window.$j.post(configuration.rating_url, {
          rating: rating,
  ...
  module.submitRatingReason = function (ratingReasonId) {
    if (configuration && configuration.rating_reason) {
      if (configuration.rating_reason.rating_reason_url) {
        window.$j.post(configuration.rating_reason.rating_reason_url, {
  ...
  module.submitNote = function (note) {
    ...
    if (configuration && configuration.note_url && configuration.note_token) {
      window.$j.post(configuration.note_url, {
        note: note,
  ...
  module.submitLocation = function (position, successCb, failureCb) {
    if (configuration && configuration.find_me_url && configuration.find_me_token) {
      window.$j.post(configuration.find_me_url, {
        position: position,
  ...
  module.submitTip = function (tip) {
    ...
    if (configuration && configuration.tipConfiguration && configuration.tipConfiguration.tipSignatureUploadPath
      ...
      window.$j.post(configuration.tipConfiguration.tipSignatureUploadPath, {
        amount: tip,
        ...
        window.$j.ajax({
          ...
          window.$j.post(configuration.tipConfiguration.tipUrl, {
            amount: tip,
            tipToken: configuration.tip_token,
            signatureImage: fileName,
            currency: configuration.tipConfiguration.tipCurrency,
  ...
  module.setUpConfigByRegion = function (region) {
    if (!region) {
      log('_setUpConfigByRegion: please provide region');
      return;
    }

    if (region) {
      log('_setUpConfigByRegion: setting up region to ' + region);
      REAL_TIME_PRODUCTION = REGIONS[region];
      REAL_TIME_OPTIONS.END_POINT = REAL_TIME_PRODUCTION;
      return true;
    }
  ...
  module.setUpConfigByRegionUrl = function (region_url) {
    if (!region_url) {
      log('_setUpConfigByRegionUrl: please provide region url');
      return;
    }

    if (region_url) {
      log('_setUpConfigByRegionUrl: setting up region url to ' + region_url);
      REAL_TIME_OPTIONS.END_POINT = region_url;
      return true;
    }
  };
  ...
  module._connectSocket = function () {
    ...
    module._socket = io(getRealTimeEndPoint(), {
      ...
      transportOptions: {
           websocket: {port: getWebSocketPort(), secure: getSecuredSocketSetup()}
       }
    });
  ...
  module.getDriverPhone = function (uuid, customerPhone, cb) {
    ...
    window.$j.get(getRealTimeEndPoint() + '/shared/' + uuid + '/phone_number', {original_phone: customerPhone})
  ...
  function customerAlert(options) {
    ...
    var data = window.$j.extend({}, {alert_type: 0, token: configuration.alerting_token}, options);

    window.$j.post(configuration.alerting_url, data, function (response) {
  ...
  function getShareConfig(uuid, callback, errorCallback) {
    ...
    window.$j.getJSON(getRealTimeEndPoint() + 'shared/' + uuid + '?full=true', callback).error(errorCallback);
  }

  function getSharedLocation(uuid) {
    log('Getting location via REST for uuid: ' + uuid);

    window.$j.getJSON(getRealTimeEndPoint() + 'shared/' + uuid + '/location/', function (result) {
  ...
  function getOrderViaRestByOrderUuid(shareUuid, orderUuid) {
    ...
    console.log('Getting order via REST with share end point: ', getRealTimeEndPoint());
    window.$j.getJSON(getRealTimeEndPoint() + 'watch/shared/' + shareUuid + '?order_uuid=' + orderUuid, function (result) {
      ...
      console.log('Rest order update failed: ' + (jqXHR.status || 503) + ', ' + errorThrown);
   ...
   function getOrderViaRestByAccessToken(shareUuid, accessToken) {
    log('Getting order via REST with share uuid: ' + shareUuid);
    window.$j.getJSON(getRealTimeEndPoint() + 'watch/shared/' + shareUuid + '?access_token=' + accessToken, function (result) {
  ...
  function createShareForOrderViaRestByAccessToken(orderUuid, customerAccessToken) {
    ...
    window.$j.getJSON(getRealTimeEndPoint() + 'shared/orders?order_uuid=' + orderUuid + '&access_token=' + customerAccessToken, function (result) {
      log('Rest order update: ' + JSON.stringify(result));
      if (result.success && result.order_update) {
        module._onOrderUpdate(result.order_update);
      }
  ...
  function onLocationUpdated(data) {
    ...
    if (data.lat && data.lng) {
      if (callbacks.locationUpdateCb) {
        ...
        callbacks.locationUpdateCb(jsonData);
      }
